...
class BotConfig(BaseConfig):
    ...
    driver: str = "~fastapi"
    """MagicBot 运行所使用的 `Driver`"""

    adapters: set[str] = {"~onebot.v11"}
    """MagicBot 所使用的 `Adapter`"""

    host: IPvAnyAddress = IPv4Address("127.0.0.1")  # type: ignore
    """MagicBot 的 HTTP 和 WebSocket 服务端监听的 IP/主机名"""

    port: int = Field(default=8120, ge=1, le=65535)
    """MagicBot 的 HTTP 和 WebSocket 服务端监听的端口"""

    debug: bool = False
    """是否以调试模式运行 MagicBot"""

    log_level: LevelName | int = "INFO"
    """配置 MagicBot 日志输出等级，可以为 `int` 类型等级或等级名称，参考 [loguru 日志等级](https://loguru.readthedocs.io/en/stable/api/logger.html#levels)"""

    log_file: LevelName | tuple[LevelName] = "ERROR"
    """MagicBot 的日志保存等级，必须为等级名称"""
