...
from magic.utils.api import random_text
from magic.utils.annotated import UserInfo
from magic.database.models import Bind, User
...
> Bot：已为您生成一次性token：magic/123456

然后，您需要复制此token，来到QQ频道
> 用户：@bot /bind magic/123456
> Bot：绑定成功
"""
...
async def token_(user: UserInfo, token: Match[str]):
    if token.available:
        if token.result == "random":
            random_token = random_text(15, prefix="Magic/")
